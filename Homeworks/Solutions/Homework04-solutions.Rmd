...
**Answer:** 
The variables that are continuous are `when.erected` and `length`. `when.erected` ranges from 1819 to 1978, `length` ranges for 840 to 4558. I consider `when.erected` as a continuous variable because time is a continuous thing in natural and there should be assumptions that bridges built in similar years would be similar, which we won't be able to capture in categories, even if we made large categories. Additionally we have a large ranges of years, so most categorical analysis would become cumbersome/ with little interpretive power.
...
span_vs_river <- ggplot(pitt_bridges,aes(y = span,x = river)) + geom_mosaic() +
                mosaic_legend() +
...
```{r}
span_vs_river_bar <- ggplot(pitt_bridges,aes(fill = span,x = river)) + 
...
**Answer:**
It appears that aqueducts are not very plentiful, and as iron bridges aren't that common period, we see potental lack of independence based on having an iron aqueduct bridge.
...
```{r}
length_box <- ggplot(pitt_bridges,aes(x = factor(""),y = length)) +
...
```{r}
length_v_material_box <- ggplot(pitt_bridges,aes(x = material,y = length)) +
...
ggplot(data = bridge_data, aes(x = when.erected)) +
  ...
  xlab("Year Bridge Erected") + 
  ggtitle("Bridges Erected in Pittsburgh by Year") +
  my_theme
```
