...
ks.test(movies$profit, dnorm, mean(movies$profit, na.rm = TRUE), 
...
A basic linear regression, regressing `Imdb score` on `budget` (in $100$ million dollar units) has a positive coefficient for `budget` of $`r round(lin_all$coef[2],4)`$, that is for every 15 million dollars of budget we'd expect an increase of the `Imdb` score by 1 unit. 

This positive relationship is significant, with a p-value of $`r round(10^4*summary(lin_all)$coef[2,4],5)` \cdot 10^{-4}$, which is much much smaller than any standard threshold.
...
Additionally,we can examining the strength of a linear model that allows for interactions between the `content rating` and `budget` slope, allowing each subset (based on `content rating`) to have different splots. Compared to a model that doesn't allow each `content rating` group to have it's ovwer `budget` slope, we obtain a p-value of $`r round(f_test,4)`$. This is slightly over a standard threshold level of $\alpha = .05$, so we conclude that allowing each `content rating` group to have their own slopes is not statically signification.

c.  (5 points)  

We decided to use a scatter plot of `budget` vs `imdb score` faceted on `content rating` adding red regression lines and confidence bands for the regression to look at linear trends. We used a scatter plot because both variables `budget` and `imdb score` are continuous and included regression lines to examine the overall relationships. 354 NA's we removed.
