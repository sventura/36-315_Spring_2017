...
date: "Due Friday, September 23, 2016 (6:30pm)"
output: 
  html_document:
    toc:  true
    toc_float:  true
...
+  Upload a single `R` Markdown file (named as:  [AndrewID]-Lab04.Rmd -- e.g. "sventura-Lab04.Rmd") to the Lab 04 submission section on Blackboard.  You do not need to upload the .html file.
...
![alt text](https://www.theatlas.com/i/atlas_41nluQRO@2x.png)
</div>
...
Read the dataset in from GitHub and repeat the same data manipulations from Homework03 (code provided).  Then complete the following questions.
...
c.  Repeat part (a), but this time, test the independence of the `day_of_week` and `birth_decade` variables.  This time, interpret the result of the chi-square test for independence **in the context of the problem**.  Do you think your result makes sense, given what we might expect about bike riders in NYC?
...
```{r, warning = F, message = F}
ggplot(bike_data, aes(x = gender, y = am_or_pm)) + geom_mosaic() + mosaic_legend()
```

Remember to add a title and appropriate axis labels to the mosaic plots.  Also, note that in this implementation, the locations of the x- and y-axis tick marks do not necessarily line up with the locations of the boxes.  We'll fix this on the homework.

Now, answer the following questions:

a.  (5 points)  Recall that mosaic plots color each box according to the standardized Pearson residuals.  Do the mosaic plots yield the same conclusions about whether the two variables being plotted in each mosaic plot are independent?
